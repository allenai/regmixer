...
def build_figure() -> plt.Figure:
    ...
    ax.text((x_mid + x_left)/2 - 1 , y_top_titles-1.0, "'Add' update", ha="center", va="center", fontsize=14, color=BLACK)
    arrow(ax, x_left+2.0, y_top_titles-1.5, x_mid-3.5, y_top_titles-1.5, ms=18, lw=1)
    ...
    legend_item(ax, legend_x, legend_y0 - 1.2, r"affected domains", facecolor=GREEN)
    legend_item(ax, legend_x, legend_y0 - 2.4, r"reuse ratios within $\mathcal{D}_{\mathrm{fix}}$", facecolor=None, outline=True)
    ...
    xL, xR, yB, yT = draw_olmixbase_call(
        ...
        center_x=x_mid+1.5,
        center_y=y_mid_blocks,
        labels=[r"$\mathcal{D}_{\mathrm{fix}}$", r"$D_2$", r"$D_4'$"],
    ...
    small_center_y = y_mid_blocks - 2.0
    ...
    xL2, xR2, yB2, yT2 = draw_olmixbase_call(
        ...
        center_y=y_mid_blocks,
        labels=[r"$\mathcal{D}_{\mathrm{fix}}$", r"$D_4'$"],
        colors=[BLUE, GREEN],
        thick_outline_idx={0},
    )
    ...
    small_center_y2 = y_mid_blocks - 2.0
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("fig_mixreuse.png"))

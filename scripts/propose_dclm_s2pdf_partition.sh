...
: 'for R in 3 4 5
do
    rmc-eval fit -c src/regmixer/config/for_paper/backfill-5xC-30m-dclm-s2pdf-one-node-conditional-s2pdf-partitioned.yaml \
        -g dcde5cda \
...
for R in 4 #3 4 5
do
    rmc-eval fit -c src/regmixer/config/for_paper/backfill-5xC-30m-dclm-s2pdf-one-node-conditional-s2pdf-partitioned-1T.yaml \
        -g 8effa6b3 \
        -G pretraining_tasks_for_paper \
        -a 1 \
        -S 100_000 \

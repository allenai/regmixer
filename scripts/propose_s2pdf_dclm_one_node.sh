...
: 'for R in 3 4 5
do
    rmc-eval fit -c src/regmixer/config/for_paper/backfill-5xC-30m-s2pdf-dclm-one-node.yaml \
        -g f5a3ff58 \
...
do
    rmc-eval fit -c src/regmixer/config/for_paper/backfill-5xC-30m-s2pdf-dclm-one-node.yaml \
        ...
        --dashboard mixing-paper \
        --constrain-objective \
        --manual-token-constraint-path src/regmixer/eval/superswarm_2T_collapse_dclm_requested_vs_available_tokens.yaml \
        --repetition-factor $R \
        --drop-metrics ultrachat_masked_ppl \
        --drop-metrics wildchat_masked_ppl \
        --drop-metrics qasper_yesno:rc::olmes \

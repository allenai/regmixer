...
for R in 3 4 5
do
    rmc-eval fit -c src/regmixer/config/for_paper/backfill-5xC-30m-dclm-stackv2-one-node-conditional-stackedu-one-node.yaml \
        -g 91afc9bd \
        ...
        --constrain-objective \
        --manual-token-constraint-path src/regmixer/eval/superswarm_collapse_dclm_s2pdf_stackedu_requested_vs_available_tokens.yaml \
        --repetition-factor $R \
